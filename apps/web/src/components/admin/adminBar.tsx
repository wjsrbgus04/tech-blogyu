  ...
  return (
    <header className="sticky top-0 z-20 border-border border-b bg-bg/85 backdrop-blur-[10px]">
      <div className="mx-auto flex max-w-[76rem] items-center gap-4 px-8 py-3.5 max-lg:px-6 max-sm:px-[1.15rem]">
        <Link href="/admin" className="inline-flex items-center gap-2 font-[620] text-[0.8125rem]">
        ...
        <nav className="ml-4 flex gap-0.5 max-sm:ml-0">
          {NAV.map((item) => (
            <Link
              ...
              className={
                pathname === item.href
                  ? 'rounded-sm bg-bg-subtle px-2.5 py-1.5 font-[560] text-[0.8125rem] text-fg'
                  : 'rounded-sm px-2.5 py-1.5 text-[0.8125rem] text-fg-muted transition-colors hover:bg-bg-subtle hover:text-fg'
              }
            >
              {item.label}
            </Link>
        ...
        <div className="ml-auto flex items-center gap-3">
          <Link
            href="/"
            className="rounded-sm px-2.5 py-1.5 text-[0.8125rem] text-fg-muted transition-colors hover:text-fg max-sm:hidden"
          >
            블로그 보기 ↗
          </Link>
          <button
            type="button"
            onClick={signOut}
            className="cursor-pointer rounded-sm px-2.5 py-1.5 text-[0.8125rem] text-fg-muted transition-colors hover:text-fg"
          >
            로그아웃
          </button>
          <span
